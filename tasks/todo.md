...
- [ ] **[FE-046]** 정보자산관리대장 엑셀 내보내기 — 실제 ISMS 컨설팅 양식 기반
  - 참조: `정보자산관리대장_triple comma_v2603.xlsx` 컬럼 구조
  - 시트별 분리 (클라우드/서버/DB/단말기/소프트웨어/정보보안시스템/웹와스/애플리케이션/문서)
  - 공통 컬럼: No, 코드, 구분코드, 자산코드, 자산명, 수량, 업무명, 상세정보
  - 관리자 정보: 담당자, 관리자, 관리부서
  - 중요도: C, I, A, 등급 (자동 계산)
  - ISMS 대상 / 컨설팅 대상 플래그
  - 시트별 추가 컬럼:
    ...
    - 서버: 운영체제(버전), IP, 위치, 성능
    - DB: DB버전, URL
    - 단말기: 구매일자, 운영체제, IP/일련번호, 사용자, 팀명, 자산히스토리
    - 소프트웨어: 상세정보
    - 정보보안시스템: IP, OS, 위치
  - 표지 시트 + 중요도평가 기준 시트 + 자산분류 시트 포함
  - 시트별 상단에 총 자산 수량 / ISMS 대상 수량 자동 집계

### 증적 카테고리 + 보고서 연동 (자산카테고리 리팩터링)
- [ ] **[FE-047]** "자산카테고리" → "증적 카테고리" 리네이밍
  - 메뉴명, 페이지 제목, i18n 키 일괄 변경
  - 페이지 상단에 역할 설명 추가 ("증적 내보내기 시 데이터를 분류할 카테고리를 관리합니다")
- [ ] **[BE-078]** 보고서 생성 → 증적 자동 연동
  - 보고서(`/reports`) 생성 시 자동으로 Archive 레코드 생성
  - Archive에 카테고리 자동 매핑 (code 기반: LICENSE, EMPLOYEE, COST 등)
  - Google Drive 설정 시 자동 업로드 (원클릭 플로우)
  - 흐름: 보고서 생성 → PDF/Excel 렌더링 → Archive 생성 → Drive 업로드

### UX 고도화 (향후)
- [ ] **[FE-048]** 노드 정렬 가이드라인 — 드래그 시 다른 노드와 x/y 정렬선 + 동일 거리 표시
- [ ] **[FE-049]** 노드 크기 맞추기 — 선택한 노드들의 너비/높이 통일 기능
