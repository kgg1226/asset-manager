...
### [2026-06-02] [dev-023] 정책과 가드는 같은 커밋에서 함께 적용
- dev-022 가 "권한 변경/시스템 설정은 SUPER_ADMIN 전용" 정책만 선언하고 가드는 Phase 2 로 미뤄,
  그 사이 모든 ADMIN 이 role 변경·시스템 설정을 그대로 수행 가능한 공백이 생겼다.
- 권한 변경 경로가 **서버 액션(app/admin/actions.ts)과 API 라우트 양쪽**에 존재할 수 있으므로,
  한쪽만 가드하면 우회된다. 권한 변경 가드는 모든 진입점에서 일괄 적용한다.
- 권한 계층은 "상위 계정을 하위가 강등/삭제/비활성화 불가"까지 막아야 무결성이 유지된다.
