...
import { ArrowLeft, Edit, Trash2, AlertCircle, Network } from "lucide-react";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!res.ok) { toast.error(t.common.noData); router.push("/cloud"); return; }
        const data = await res.json();
        setAsset(data);
        setNewStatus(data.status);
      } catch { toast.error(t.toast.saveFail); router.push("/cloud"); }
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <div className="rounded-lg bg-white p-6 shadow-sm">
            <p className="text-sm text-gray-600">{t.cloud.platform}</p>
        ...
        {asset.cloudDetail && (() => {
          ...
          return (
            <div className="mb-8 rounded-lg bg-white p-6 shadow-sm">
              <h2 className="mb-4 text-lg font-bold text-gray-900">{t.cloud.title}</h2>
        ...
        {isAdmin && (
          <div className="flex gap-3">
            ...
            <Link href={`/cloud/${asset.id}/edit`} className="flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"><Edit className="h-4 w-4" />{t.common.edit}</Link>
