...
export default function AssetClassificationsPage() {
  ...
  async function saveMajor() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error ?? t.classification.saveFail);
      await load();
      cancelMajorForm();
    } catch (e) {
      setError(e instanceof Error ? e.message : t.classification.errorOccurred);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function deleteMajor(cat: MajorCategory) {
    ...
    const msg = totalAssets > 0
      ? `"${cat.name}" ${t.classification.confirmDeleteMajor}\n(${t.classification.linkedAssets} ${totalAssets} - ${t.classification.confirmDeleteMajorWithAssets})`
      : `"${cat.name}" ${t.classification.confirmDeleteMajor}`;
    ...
    try {
      ...
      alert(e instanceof Error ? e.message : t.classification.deleteFail);
    }
  }
  ...
  async function deleteSub(sub: SubCategory) {
    if (!confirm(`"${sub.name}" ${t.classification.confirmDeleteSub}`)) return;
    try {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                {t.classification.majorCategory} {totalMajor} · {t.classification.subCategory} {totalSub} · {t.classification.linkedAssets} {totalAssets}{t.classification.countItems}
              </p>
            </div>
          </div>
          {!showMajorForm && (
        ...
        {showMajorForm && (
          <div className="mb-6 rounded-lg bg-white p-6 shadow-sm ring-1 ring-gray-200">
            <h2 className="mb-4 text-base font-semibold text-gray-900">
              {editingMajorId ? t.classification.editMajor : t.classification.createMajor}
            </h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div>
                <label className="block text-xs font-medium uppercase text-gray-500 mb-1">
                  {t.classification.majorName} <span className="text-red-500">*</span>
                </label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium uppercase text-gray-500 mb-1">
                  {t.classification.code} <span className="text-red-500">*</span>
                </label>
                <input
              ...
              <div>
                <label className="block text-xs font-medium uppercase text-gray-500 mb-1">{t.classification.abbr}</label>
                <input
              ...
              <div className="sm:col-span-3">
                <label className="block text-xs font-medium uppercase text-gray-500 mb-1">{t.common.description}</label>
                <input
                  type="text" value={majorForm.description}
                  onChange={(e) => setMajorForm((p) => ({ ...p, description: e.target.value }))}
                  placeholder={t.hw.notes}
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none"
                />
              </div>
            </div>
            ...
            <div className="mt-4 flex gap-2">
              <button onClick={saveMajor} disabled={saving}
                ...
                <Check className="h-4 w-4" />{saving ? t.classification.saving : t.common.save}
              </button>
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 bg-gray-50 border-b border-gray-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-400 ml-2">
                          {t.classification.subCategory} {cat.subCategories.length} · {t.classification.assetCount} {subAssets}{t.classification.countItems}
                        </span>
                      </div>
                      {cat.description && (
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => startSubCreate(cat.id)}
                        ...
                        title={t.classification.addSub}>
                        <Plus className="h-3 w-3" />{t.classification.subCategory}
                      </button>
                      <button onClick={() => startMajorEdit(cat)} className="text-gray-400 hover:text-blue-500" title={t.common.edit}>
                        <Pencil className="h-4 w-4" />
                      </button>
                      <button onClick={() => deleteMajor(cat)} className="text-gray-400 hover:text-red-500" title={t.common.delete}>
                    ...
                    <div>
                      ...
                      {showSubForm === cat.id && (
                        <div className="border-b border-gray-200 bg-indigo-50/30 px-6 py-4">
                          <h3 className="mb-3 text-sm font-semibold text-gray-700">
                            {editingSubId ? t.classification.editSub : `"${cat.name}" ${t.classification.addSub}`}
                          </h3>
                          <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
                            <div>
                              <label className="block text-xs font-medium text-gray-500 mb-1">
                                {t.classification.subName} <span className="text-red-500">*</span>
                              </label>
                          ...
                          <div className="mt-3 flex items-center gap-6">
                            ...
                            <label className="inline-flex items-center gap-2 text-sm">
                              ...
                              {t.classification.consultingTarget}
                            </label>
                          </div>
                          ...
                          <div className="mt-3 flex gap-2">
                            <button onClick={saveSub} disabled={saving}
                              ...
                              <Check className="h-3.5 w-3.5" />{saving ? t.classification.saving : t.common.save}
                            </button>
                      ...
                      {cat.subCategories.length === 0 ? (
                        <div className="px-6 py-4 text-center text-sm text-gray-400">
                          {t.classification.noSubCategories}
                        </div>
                      ) : (
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="text-xs font-medium uppercase text-gray-400 border-b border-gray-100">
                              ...
                              <th className="px-2 py-2 text-left">{t.classification.code}</th>
                              <th className="px-2 py-2 text-left">{t.classification.subName}</th>
                              <th className="px-2 py-2 text-center">ISMS-P</th>
                              <th className="px-2 py-2 text-center">{t.classification.consultingTarget}</th>
                              <th className="px-2 py-2 text-center">{t.classification.assetCount}</th>
                              <th className="px-2 py-2 text-right pr-4">{t.classification.management}</th>
                            </tr>
                          </thead>
                              ...
                              <tr key={sub.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-2 py-2 text-right pr-4">
                                  <div className="inline-flex gap-2">
                                    <button onClick={() => startSubEdit(sub)} className="text-gray-400 hover:text-blue-500" title={t.common.edit}>
                                      <Pencil className="h-3.5 w-3.5" />
                                    </button>
                                    <button onClick={() => deleteSub(sub)} className="text-gray-400 hover:text-red-500" title={t.common.delete}
