...
export default function ExternalEntityEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mx-auto max-w-2xl">
        {/* Header */}
        <div className="mb-6">
          <Link href={`/external/${id}`} className="mb-2 inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
            <ArrowLeft className="h-4 w-4" /> {t.externalEntity.detail}
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">{t.externalEntity.edit}</h1>
        </div>
